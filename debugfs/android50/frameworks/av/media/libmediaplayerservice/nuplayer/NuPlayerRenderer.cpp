...
#if 1
     fp = fopen("/data/misc/audio/NuPlayerdump.pcm", "a+");
     if (fp == NULL) {
...
void NuPlayer::Renderer::postDrainAudioQueue_l(int64_t delayUs) {
    if (mDrainAudioQueuePending || mSyncQueues || mPaused
             || mAudioSinkStopped ||offloadingAudio()) {//for bug 83312,meger google patch,2015.07.24 
        return;
    }
...
bool NuPlayer::Renderer::onDrainAudioQueue() {
    ...
    while (numBytesAvailableToWrite > 0 && !mAudioQueue.empty()) {
        ...
        if (entry->mBuffer == NULL) {
            ...
            mAudioQueue.erase(mAudioQueue.begin());
            entry = NULL;
            //for bug 73476,meger patch from qcom 2015.07.08 begin
            if (mAudioSink->needsTrailingPadding()) {
				...
				mAudioSinkStopped = true;
                mNumFramesWritten = 0;
                ALOGV("onDrainAudioQueue mAudioSink->stop");
            }
            //for bug 73476,meger patch from qcom 2015.07.08 end
            return false;
        }
		...
		if(written > 0)
		{
	        write_to_file_for_audio_2s((char *)entry->mBuffer->data() + entry->mOffset
								, written,2000,AUDIO_FORMAT_PCM_16_BIT);
		}
#endif
        if (written < 0) {
        ...
        if (written != (ssize_t)copy) {
            ...
            //+Other, zhulihua.wt, 20150803, modify, can't play popstar game
            entry->mNotifyConsumed->post();
            mAudioQueue.erase(mAudioQueue.begin());
            entry = NULL;
            //-Other, zhulihua.wt, 20150803, modify, can't play popstar game
...
//#define WAVES_PROCESSING
#ifdef WAVES_PROCESSING
                if (offloadOnly)
                {
                    // @ 2015.4.4
                    // NuPlayer failed to recover from this situation.
                	// If the creation of an Offloaded sink failed, it should also tear down the DecoderpassThrough object.
                	// Force an tear down here.
                	// If NuPlayer fixed this issue, we will come back and remove this code.
                    ALOGV("Waves: openAudioSink: offload failed on offloadOnly request. Teardown the offload mode");
                    mAudioOffloadTornDown = false;
                    notifyAudioOffloadTearDown();
                }
#endif
