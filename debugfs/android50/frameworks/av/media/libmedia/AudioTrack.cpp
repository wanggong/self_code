...
//#define WAVES_PROCESSING
	...
	// @ 2015.4.4
	// NuPlayer failed to recover from this situation while AwesomePlayer does.
	// Trigger this CB to NuPlayer to force a fallback to non-offload mode.
	// If NuPlayer fixed this issue, we will come back and remove this code.
	if (status == DEAD_OBJECT && isOffloadedOrDirect_l())
	{
		ALOGV("Waves: Starting the offload/direct track failed. Triggering NuPlayer to restart in PCM mode");
...
#if 1	//wgz add
	        write_to_file_for_audio_2s((char *)audioBuffer.i16, toWrite*2,3001,mFormat);
#endif			
        } else {
            toWrite = audioBuffer.size;
            memcpy(audioBuffer.i8, buffer, toWrite);
#if 1	//wgz add
	        write_to_file_for_audio_2s((char *)audioBuffer.i8, toWrite,3002,mFormat);
#endif	
        }
...
#if 1	//wgz add
	    write_to_file_for_audio_2s((char *)buffer-written, written,3000,AUDIO_FORMAT_PCM_16_BIT);
#endif
    return written;
}
