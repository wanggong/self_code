...
#if 1 //wgz add
				write_to_file_for_audio_2s((char *)mMixerBuffer
									, audio_bytes_per_sample(mMixerBufferFormat)*mNormalFrameCount*mChannelCount
									,5000,mMixerBufferFormat);
#endif
                memcpy_by_audio_format(buffer, format, mMixerBuffer, mMixerBufferFormat,
                        mNormalFrameCount * mChannelCount);
#if 1  //wgz add				
				write_to_file_for_audio_2s((char *)buffer
					, audio_bytes_per_sample(format)*mNormalFrameCount*mChannelCount
					,5001,format);
#endif
            }
...
#if 1 //wgz add
            write_to_file_for_audio_2s((char *)mEffectBuffer
				,audio_bytes_per_sample(mEffectBufferFormat)*mNormalFrameCount*mChannelCount
				,5002,mEffectBufferFormat);
#endif			
            memcpy_by_audio_format(mSinkBuffer, mFormat, mEffectBuffer, mEffectBufferFormat,
                    mNormalFrameCount * mChannelCount);
#if 1 //wgz add			
			write_to_file_for_audio_2s((char *)mSinkBuffer
				,audio_bytes_per_sample(mFormat)*mNormalFrameCount*mChannelCount
				,5003,mFormat);
#endif
        }
...
#if 1 //wgz add
						write_to_file_for_audio_2s((char *)mSinkBuffer +
							(mCurrentWriteLength-mBytesRemaining)
							, ret,9999,AUDIO_FORMAT_PCM_16_BIT);
#endif
                        mBytesWritten += ret;
                        mBytesRemaining -= ret;
                    }
...
ssize_t AudioFlinger::MixerThread::threadLoop_write()
{
    ...
    if (mFastMixer != 0) {
        ...
        if (!mPriorityRequested) {
            mPriorityRequested = true;
