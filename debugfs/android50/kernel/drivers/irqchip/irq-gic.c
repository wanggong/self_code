...
#ifdef CONFIG_PM
#if 1 //wgz add 
void (*resume_before_gic_irq_resume)(void) = 0 ;
void (*suspend_after_gic_irq_suspend)(void) = 0 ;
EXPORT_SYMBOL_GPL(resume_before_gic_irq_resume);
EXPORT_SYMBOL_GPL(suspend_after_gic_irq_suspend);
extern unsigned int gic_wakeup_irqs[32];
#endif //wgz add end
...
#if 1 //wgz add
		writel_relaxed(gic->wakeup_irqs[i]&(~gic_wakeup_irqs[i]),
...
#if 1 //wgz add
	if(suspend_after_gic_irq_suspend != NULL)
	{
		suspend_after_gic_irq_suspend();
	}
#endif //wgz add end
	return 0;
}
...
#if 1 //wgz add
	if(resume_before_gic_irq_resume != NULL)
	{
		resume_before_gic_irq_resume();
	}
#endif  //wgz add end
...
#if 1 //wgz add
static int print_hex(int *data, int count)
{
	int index = 0;
...
void print_enabled_name(int *data , int count)
{
	int  i ,j  , raw_data, hw_irq , desc_irq;
	struct gic_chip_data *gic = &gic_data[0];
	...
	for(i = 0 ; i < count ; ++i)
	{
		...
		for(j = 0 ; j < 32 ;j++)
		{
			...
			{
				hw_irq = i * 32 + j;
				if (likely(hw_irq > 15 && hw_irq < 1021)) 
				{
					struct irqaction	*action;
					desc_irq = irq_find_mapping(gic->domain, hw_irq);
					desc = irq_to_desc(desc_irq);
					printk(KERN_EMERG"hw_irq = %d , desc_irq = %d,name=" , hw_irq , desc_irq);
					action = desc->action;
					while(action != NULL)
					{
						printk(KERN_EMERG"(%s)" , action->name);
						action = action->next;
					}
					printk(KERN_EMERG"\n");
				}
...
static unsigned int enable_gic_buffer[128] = {0}; 
static unsigned int pending_gic_buffer[128] = {0}; 
static unsigned int active_gic_buffer[128] = {0}; 
static unsigned int en_pen_gic_buffer[128] = {0}; 



void dump_gic_register(int off , int count , int *dump_gic_buffer)
{
	int i = 0;
	void __iomem *base = gic_data_dist_base(&gic_data[i]);
	raw_spin_lock(&irq_controller_lock);
	for( i = 0 ; i < count/4 ; i++)
	{
		dump_gic_buffer[i] = readl(base + off + i*4);
	}
	raw_spin_unlock(&irq_controller_lock);
	print_hex(dump_gic_buffer , count/4);
	print_enabled_name(dump_gic_buffer , count/4);
}

void dump_gic_combine_register(unsigned int *dump_gic_buffer1 , unsigned int *dump_gic_buffer2 ,  int count)
{
	int i = 0;
	for(i = 0 ; i < 0x80/4; i++)
	{
		en_pen_gic_buffer[i] = dump_gic_buffer1[i]&dump_gic_buffer2[i];
	}
	print_hex(en_pen_gic_buffer , count/4);
	print_enabled_name(en_pen_gic_buffer , count/4);
}
void debugfs_dump_gic(void)
{
	printk(KERN_EMERG"\n\n\n\n\n--------------------------dump gic interrupts-----------------------------\n");
	printk(KERN_EMERG"\n enable register\n");
	dump_gic_register(GIC_DIST_ENABLE_SET , 0x80 , enable_gic_buffer);
	printk(KERN_EMERG"\n pending register\n");
	dump_gic_register(GIC_DIST_PENDING_SET , 0x80 , pending_gic_buffer);
	printk(KERN_EMERG"\n active register\n");
	dump_gic_register(GIC_DIST_ACTIVE_SET , 0x80 , active_gic_buffer);
	printk(KERN_EMERG"\n enable and pending register\n");
	dump_gic_combine_register(enable_gic_buffer , pending_gic_buffer , 0x80);
	printk(KERN_EMERG"\n ----------------------------------dump gic interrupts end--------------------\n\n\n\n\n");
}
#endif //wgz add end
