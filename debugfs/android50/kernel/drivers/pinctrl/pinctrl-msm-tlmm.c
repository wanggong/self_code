...
#if 1 //wgz add
struct msm_tlmm_desc *tlmm_desc_debug = 0;

void msm_tlmm_v4_gp_irq_show(int irq,struct  msm_tlmm_irq_chip *ic)
{
	unsigned long i;
	unsigned int virq = 0;
	...
	for_each_set_bit(i, ic->enabled_irqs, ic->num_irqs) {
		dev_dbg(ic->dev, "hwirq in bit mask %d\n", (unsigned int)i);
		if (msm_tlmm_get_intr_status(ic, i)) {
			printk(KERN_EMERG"hwirw %d fired\n", (unsigned int)i);
			virq = msm_tlmm_gp_to_irq(gc, i);
			if (!virq) 
			{
				dev_dbg(ic->dev, "invalid virq\n");
			...
			if(desc2->action&&desc2->action->name)
			{
				printk(KERN_EMERG"irq=%d,name=%s\n" , virq ,desc2->action->name);
			}
			else
			{
...
void debugfs_msm_tlmm_v4_gp_irq_show(void)
{
	if(tlmm_desc_debug!=NULL)
	{
		int i, num_pintypes;
		struct msm_pintype_info *pintypes, *pintype;
		struct msm_tlmm_irq_chip *ic;
		struct msm_tlmm_desc *tlmm_desc = (struct msm_tlmm_desc *)tlmm_desc_debug;
		int irq = tlmm_desc->irq;
		...
		for (i = 0; i < num_pintypes; i++) 
		{
			...
			if (!ic->node)
				continue;
			msm_tlmm_v4_gp_irq_show(irq, ic);
		}
		printk(KERN_EMERG"-------------------------gpio irq end------------------------\n\n\n\n\n");
	}
}
EXPORT_SYMBOL_GPL(debugfs_msm_tlmm_v4_gp_irq_show);
#endif //wgz add end
...
#if 1 //wgz add
void (*resume_before_msm_tlmm_v4_gp_irq_resume)(void) = 0 ;
void (*suspend_after_msm_tlmm_v4_gp_irq_suspend)(void) = 0 ;

EXPORT_SYMBOL_GPL(resume_before_msm_tlmm_v4_gp_irq_resume);
EXPORT_SYMBOL_GPL(suspend_after_msm_tlmm_v4_gp_irq_suspend);
#endif //wgz add end
static int msm_tlmm_gp_irq_suspend(void)
{
	...
	spin_unlock_irqrestore(&ic->irq_lock, irq_flags);
#if 1 //wgz add
	if(suspend_after_msm_tlmm_v4_gp_irq_suspend != NULL)
	{
		suspend_after_msm_tlmm_v4_gp_irq_suspend();
	}
#endif //wgz add end
	return 0;
}
...
static void msm_tlmm_gp_irq_resume(void)
{
	...
	int num_irqs = ic->num_irqs;
#if 1 //wgz add
	if(resume_before_msm_tlmm_v4_gp_irq_resume != NULL)
	{
		resume_before_msm_tlmm_v4_gp_irq_resume();
	}
#endif //wgz add end
	spin_lock_irqsave(&ic->irq_lock, irq_flags);
	for_each_set_bit(i, ic->wake_irqs, num_irqs)
