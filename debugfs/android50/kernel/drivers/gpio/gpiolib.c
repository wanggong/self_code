...
static void gpiolib_debug_show_detail(struct gpio_chip *chip)
{
	unsigned		i;
	unsigned		gpio = chip->base;
	struct gpio_desc	*gdesc = &chip->desc[0];
...
static void gpio_debug_show_chip_info(struct gpio_chip *chip )
{
	struct device *dev;
...
void gpio_debug_show_all(void)
{
	...
	{
		pr_err("----------------------------------------------------\n");
		gpio_debug_show_chip_info(chip);
		gpiolib_debug_show_detail(chip);
		pr_err("----------------------------------------------------\n\n");
	}
	spin_unlock_irqrestore(&gpio_lock, flags);
}
