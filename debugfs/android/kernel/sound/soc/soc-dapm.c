...
static void dump_widget_status(struct snd_soc_dapm_widget *w,char *start_with)
{
	int powered = 0;
	int i = 0;
	const struct snd_kcontrol_new *kcon = NULL;
	if(w->power_check != NULL)
	{
		powered = w->power_check(w);
	}
	printk(KERN_EMERG"wgz_widget: %s,name:%s,sname:%s,con:%d,active:%d,id:%d,powered:%d,input:%d,output:%d,num_kcontrols=%d\n",
		start_with,w->name,w->sname,w->connected,w->active,w->id,powered,w->inputs,w->outputs,w->num_kcontrols);
	for(i = 0 ; i < w->num_kcontrols; i++)
	{
		kcon = &w->kcontrol_news[i];
		printk("kcontrol name:%s,iface=0x%x\n",kcon->name,kcon->iface);
	}
}

int dump_widget_set = 1;
static void dump_widget(struct snd_soc_dapm_widget *w)
{
	struct snd_soc_dapm_path *p = NULL;
	int powered = 0;
	struct snd_soc_dapm_widget *temp = NULL;

	if(dump_widget_set == 0)
	{
		return;
	}
	if(w->power_check != NULL)
	...
	temp = w;
	dump_widget_status(w,"widget");
	list_for_each_entry(p, &w->sources, list_sink) 
	{	
		dump_widget_status(p->source,"source");
	}
	list_for_each_entry(p, &w->sinks, list_source) 
	{
		dump_widget_status(p->source,"sink");
	}
	printk(KERN_EMERG"\n");
}
...
int dump_all_widgets_set = 0;

static int dump_all_widgets(struct snd_soc_card *card )
{
	struct snd_soc_dapm_widget *w = 0;
	dump_all_widgets_set = 0;
	pr_err("\n\n\n-----------------------------dump all widget---------------------------------");
	list_for_each_entry(w, &card->widgets, list)
	{
		dump_widget(w);
	}
	pr_err("-----------------------------dump all widget end---------------------------------\n\n\n");
	return 0;
}
static int dapm_power_widgets(struct snd_soc_dapm_context *dapm, int event)
{
	...
	if(dump_all_widgets_set)
	{
		dump_all_widgets(dapm->card);
	}
