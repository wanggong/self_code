...
static int reg_debug_consumers_show_one(struct regulator_dev *rdev)
{
	struct regulator *reg;
	char *supply_name;
...
void reg_debug_consumers_show_all(void)
{
	struct regulator_dev *rdev;
	pr_err("---------------regulator dump--------------------------------\n");
	mutex_lock(&regulator_list_mutex);
	list_for_each_entry(rdev, &regulator_list, list) 
	{
		reg_debug_consumers_show_one(rdev);
	}
	mutex_unlock(&regulator_list_mutex);
	pr_err("---------------regulator dump end-----------------------\n\n");
}
