...
static struct snd_soc_codec *codec_for_debug = NULL;
int msm8x16_wcd_write_for_debug(unsigned int reg,unsigned int value)
{
	int ret = -1;
	if(codec_for_debug != NULL)
	{
		return msm8x16_wcd_write(codec_for_debug, reg , value);
	}
	else
	{
		pr_err("wgz write error\n");
	}
	return ret;
}
EXPORT_SYMBOL_GPL(msm8x16_wcd_write_for_debug);


unsigned int msm8x16_wcd_read_for_debug(unsigned int reg)
{
	int ret = -1;
	if(codec_for_debug != NULL)
	{
		return msm8x16_wcd_read(codec_for_debug , reg);
	}
	else
	{
		pr_err("wgz read error\n");
	}
	return ret;
}
EXPORT_SYMBOL_GPL(msm8x16_wcd_read_for_debug);
