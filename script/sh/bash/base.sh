...
gic_wakeup_irqs=$suspend_resume_dir"/gic_wakeup_irqs"
...
function read_value()
{
	...
	if((${#read_value_returned}==19))
	then
	read_value_returned=${read_value_returned:0:18};
	fi
...
function io_value_get()
{
	...
	io_value_get_returned=$read_value_returned;
